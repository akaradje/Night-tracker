...
# --- 1. ตั้งค่าหน้าเว็บ ---
st.set_page_config(page_title="NIGHT Tracker (Total Value)", page_icon="🌙", layout="wide")
...
# --- Function: คำนวณสถานะ (เช็ค Tx ID เพื่อกรอง Alert) ---
...
# MAIN UI
# ==============================================================================
st.title("🌙 NIGHT Tracker (Total Value)")
...
if not os.path.exists(CACHE_FILE):
    ...
    grand_ready = 0
    grand_alloc = 0  # ยอดทั้งหมด (รวม Redeemed)
    grand_left = 0
    ...
    for item in cached.get("wallets", []):
        if item.get('status') == 'ok':
            ...
            for t in thaws:
                ...
                # รวมยอด (Alloc คือยอดทั้งหมดที่เคยได้รับ)
                ...
                # Alert: เฉพาะที่ยังไม่เคลม (Urgent=True และไม่ใช่ Redeemed)
    ...
    # Card 3: Total Value (Calculated from Alloc)
    total_val_thb = grand_alloc * p_thb
    m3.markdown(f'<div class="metric-card value-card"><h5>📦 มูลค่ารวมทั้งพอร์ต (Alloc)</h5><h2>฿{total_val_thb:,.2f}</h2><small>{grand_alloc:,.2f} NIGHT</small></div>', unsafe_allow_html=True)
